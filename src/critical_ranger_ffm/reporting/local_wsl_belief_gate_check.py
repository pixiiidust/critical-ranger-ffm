...
from critical_ranger_ffm.reporting.paired_signal_report import load_paired_signal_rows
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if _looks_like_fixture_provider(args.sample_provider):
            raise ValueError("fixture sample providers are not valid belief-gate evidence")
        ...
        signal_artifacts = write_paired_switch_point_artifacts(
            samples,
            output_dir,
            PairedSwitchPointConfig(
                readout_horizon_steps=args.readout_horizon_steps,
        ...
        rows = load_paired_signal_rows(signal_artifacts["csv"])
        belief_artifacts = write_belief_gate_artifacts(
            rows,
            output_dir,
            BeliefGateConfig(
                target_valid_pairs=args.target_valid_pairs,
                attempted_pair_cap=args.attempted_pair_cap,
                min_independent_seeds=args.min_independent_seeds,
                max_valid_pairs_per_seed=args.max_valid_pairs_per_seed,
                max_seed_share=args.max_seed_share,
                default_readout_horizon_steps=args.readout_horizon_steps,
            ),
        )
        summary = json.loads(belief_artifacts["json"].read_text(encoding="utf-8"))
        _validate_summary(summary)
        _print_review_summary(belief_artifacts, summary)
        return 0
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python3 -m critical_ranger_ffm.reporting.local_wsl_belief_gate_check",
        description=(
            "Local WSL 500-valid-pair belief gate. Writes paired_signal.csv, "
            "belief_gate_report.md, and belief_gate_summary.json. Not for VPS runtime."
        ),
    )
    ...
    parser.add_argument("--target-valid-pairs", type=int, default=500)
    parser.add_argument("--attempted-pair-cap", type=int, default=750)
    parser.add_argument("--min-independent-seeds", type=int, default=50)
    parser.add_argument("--max-valid-pairs-per-seed", type=int, default=10)
    parser.add_argument("--max-seed-share", type=float, default=0.05)
    parser.add_argument("--readout-horizon-steps", type=int, default=512)
    parser.add_argument("--seed-start", type=int, default=3701)
    parser.add_argument("--run-id", default="issue49-local-wsl-belief-gate")
    parser.add_argument("--config-id", default="local-wsl-belief-gate-v1")
    return parser
...
def _validate_summary(summary: dict[str, Any]) -> None:
    ...
    if verdict not in VALID_BELIEF_GATE_VERDICTS:
        raise ValueError(f"summary verdict must be one of {sorted(VALID_BELIEF_GATE_VERDICTS)}")


def _print_review_summary(artifacts: dict[str, Path], summary: dict[str, Any]) -> None:
    print("#49 local WSL 500-valid-pair belief-gate summary")
    ...
    print(f"max_valid_pairs_per_seed={summary['max_valid_pairs_per_seed']}")
    print(f"max_seed_share={summary['max_seed_share']:.3f}")
    ...
    print("evidence_label=provisional_belief_gate")
    print("non_claim=not final criticality; not SOC control; not publication-grade science; not policy quality")
